...
function draw() {
  ...
  if (settings.redraw_bg) off.background(255);
  ...
  // see the offscreen canvas
  imageMode(CENTER);
  //scale(0.5);
  //translate(width/2, height/2);;
  image(off, 0, height/2);
  translate(width, 0);
  scale(-1, 1);
  image(off, 0, height/2);
...
function render(agent) {
  ...
  let s = map(n, -1, 1, 0.5, 1);
  
  off.push();
  off.translate(agent.pos.x, agent.pos.y);
  off.rotate(agent.vel.heading() + PI/2);
  off.scale(s);
